...
function addDays(date, days) {
  ...
  const inputDate = date.toISOString().slice();

  date.setDate(date.getDate() + days);

  const resultDate = date.toISOString().slice();
...
function getAge(birthDate) {
  ...
  const formattedBirthDate = birthDate.toISOString().slice();
...
function compareDates(date1, date2) {
  ...
  const formattedDate1 = date1.toISOString().slice();
  const formattedDate2 = date2.toISOString().slice();
...
function getDaysDifference(startDate, endDate) {
  ...
  // Збереження дат для виведення в форматі ISO.
  const formattedStartDate = startDate.toISOString().slice();
  const formattedEndDate = endDate.toISOString().slice();
